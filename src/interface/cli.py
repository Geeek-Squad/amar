...
class CLI:
    ...
    def start(self):
        ...
        greeting = "Hello. I am listening." if self.voice else "Hello. Type below."
        ...
        while self.running:
            try:
                ...
                if self.voice:
                    ...
                    if not user_input:
                        # Logic to not spam console if nothing heard
                        continue
                ...
                # Check for Exit
                if any(x in user_input.lower() for x in ["exit", "quit", "bye", "chup", "stop listening"]):
                    self.type_out("[AMAR]: Goodnight.")
                    self.running = False
                    break
    ...
    def process_input(self, text: str):
        ...
        if decision.type == DecisionType.ACT:
            if decision.action_plan == "chat_response":
                # Use the response text generated by the brain
                kwargs = decision.action_args or {}
                reply = kwargs.get("response_text", "I am listening.")
                self.type_out(f"[AMAR]: {reply}")
            else:
